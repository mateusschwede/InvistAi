        ...
        <div class="row">
            <div class="col-sm-12 text-center">
                <h2 style='color: blue;'>Investir na carteira <?= $_SESSION['idCarteira'] ?>:
                    ...
                    foreach ($linhas as $l) {
                        echo "  " . $l['objetivo'] . "</h2>";
                        $objCarteira = $l['objetivo'];
                        $percCart = $l['percInvestimento'];
                        $qac == count($linhas);
                    }
                    ?>
                            ...
                            <tbody>
                                ...
                                foreach ($linhas3 as $l3) {
                                    ...
                                    foreach ($linhas4 as $l4) {
                                        $cotacaoAtualAcao = $l4['cotacaoAtual'];
                                    }
                                ...
                                foreach ($linhas as $l) { //Pegar dados específicos da ação citada
                                    ...
                                    $r = $db->prepare("SELECT * FROM carteira_acao WHERE idCarteira=?");
                                    $r->execute(array($_SESSION['idCarteira']));
                                    $lin = $r->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($lin as $z) {
                                ...
                                $p = 0;
                                $qta = array();
                                for ($q = 1; $q < count($lin) + 1; $q++) {
                                    while ($aporte / $cot[$y] >= 1) {
                                        for ($x = 1; $x < count($lin) + 2; $x++) {
                        ...
                        <table class='table table-striped' style="font-size: 90%; text-align: right; line-height: 95%;">
                            <tbody>
                                <tr>
                                    <?php
                                    echo "       
                                        <td class='setx' colspan='4' style='color: blue;'><b>Total Patr Atual:</b> R$ " . number_format($valornacart, 2, ",", ".") . "
                                             </td>
                                        ...
                                        <td class='setx' colspan='4' style='color: brown;'><b>Investimento Projetado:</b> R$ " . number_format($valornacart + $_SESSION['valorInvestimento'] - $aporte, 2, ",", ".") . " 
                                            </td>
                                        <tr>
                                        ";
                                    ?>
